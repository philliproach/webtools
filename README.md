...
- A wide range of client-side tools located under `src/tools/`, including (but not limited to):
  - Annotator, Base64 Encoder, BMI Calculator, Calculator, Clipboard Helper
...
## Why use WebTools over other sites
...
- Easy to extend — new tools follow the same layout and are simple to add as standalone pages under `src/tools/`.
...
- Adding a new small, focused tool (create a new folder under `src/tools/` with a single `index.html`) 
