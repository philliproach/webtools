document.addEventListener('DOMContentLoaded', function () {
            ...
            try {
                if (format === 'image/gif') {
                    if (file.type === 'image/gif') {
                        results.push({ name: file.name.replace(/\.[^.]+$/, '') + '.gif', blob: file });
                        continue;
                    } else {
                ...
                if (format === 'image/svg+xml') {
                    ...
                    results.push({ name: file.name.replace(/\.[^.]+$/, '') + '.svg', blob: svgBlob });
                    continue;
                }

                if (format === 'image/bmp') {
                    const bmpBlob = await encodeBMP(img);
                    results.push({ name: file.name.replace(/\.[^.]+$/, '') + '.bmp', blob: bmpBlob });
                    continue;
                }

                if (format === 'image/x-icon') {
                    const icoBlob = await encodeICO(img);
                    results.push({ name: file.name.replace(/\.[^.]+$/, '') + '.ico', blob: icoBlob });
                    continue;
                }
    ...
    function encodeBMP(img) {
        return new Promise((resolve, reject) => {
            ...
            view.setUint8(offset, 0x42); offset++;
            view.setUint8(offset, 0x4D); offset++;
            ...
            view.setInt32(offset, h, true); offset += 4;
